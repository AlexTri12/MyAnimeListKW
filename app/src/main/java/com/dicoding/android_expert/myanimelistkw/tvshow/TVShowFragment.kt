...
import com.dicoding.android_expert.myanimelistkw.databinding.FragmentTVShowBinding
import com.dicoding.android_expert.myanimelistkw.detail.DetailActivity
import org.koin.android.viewmodel.ext.android.viewModel

class TVShowFragment(private val fromActivity: String) : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (activity != null) {
            ...
            if (fromActivity == HOME_ACTIVITY) {
                getDataForMainActivity()
            } else if (fromActivity == FAVORITE_ACTIVITY) {
    ...
    private fun getDataForMainActivity() {
        viewModel.tvShowList.observe(viewLifecycleOwner, { animeList ->
            if (animeList != null) {
                when(animeList) {
                    is Resource.Loading -> {
